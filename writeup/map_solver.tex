...
Each lidar endpoint is assigned an isotropic 2D covariance. For a point measured at range $r$, define
\begin{equation}
\sigma^2(r) = \sigma_r(r)^2 + \left(r\sigma_\theta\right)^2,
\end{equation}
where $\sigma_r(r)$ is the range uncertainty model and $\sigma_\theta$ is the angular uncertainty. For a transformed source point $p_l$ and target point $q_j$, the combined variance is
\begin{equation}
\sigma_{lj}^2 = \sigma_p^2(r_l) + \sigma_q^2(r_j),
\end{equation}
where $r_l$ and $r_j$ are the original lidar ranges associated with points $p_l$ and $q_j$. The Gaussian covariance is then
\begin{equation}
...
H = \nabla^2 J(\hat T)
\end{equation}
is the Hessian of the negative log likelihood. The relative-pose covariance is approximated by
\begin{equation}
\Sigma_{ik} \approx H^{-1},
\end{equation}
provided $H$ is positive definite and well conditioned. The Hessian is estimated numerically using centered finite differences of $J(x,y,\theta)$ evaluated near the optimum.
